/**
 * R2-R3 Admin Session Cookie Playwright Verification
 *
 ...
 * - UI Product page = 200
 * - UI Order page = 200
 * - Receipt/Entitlement read-only route = 200
...
const PRODUCT_ID = process.env.PRODUCT_ID || "prod_01M1DKD1NYMRWJW5R1K2WD0VG2";
const ORDER_ID = process.env.ORDER_ID || "order_01M1DKD1Z2153RYJH8M7D03HAD";
const RUN_ID = process.env.RUN_ID || "x2_951abc2715fc9be2";
const OUT_DIR = process.env.OUT_DIR || path.resolve("governance/r2r3");
...
async function main() {
  ...
  const productRendered = productHtml.includes("Synthetic Digital Checklist");
  ...
  const orderRendered = orderTitle.includes("#1") || (await page.content()).includes(ORDER_ID);

  // Step 8: Verify Receipt / Entitlement read-only route
  ...
  // Step 9: Refresh session persistence
  ...
  const evidence = {
    schema_version: 1,
    evidence_type: "R2R3_ADMIN_SESSION_EVIDENCE",
    ...
    verdict: (
      ...
      pageErrors.length === 0
    ) ? "R2R3_ADMIN_SESSION_PASS" : "R2R3_ADMIN_SESSION_FAIL"
  };

  const evidencePath = path.join(OUT_DIR, "R2R3_ADMIN_SESSION_EVIDENCE.json");
  ...
  fs.writeFileSync(evidencePath + ".sha256", `${sha}  R2R3_ADMIN_SESSION_EVIDENCE.json\n`, "ascii");

  console.log("R2R3_ADMIN_SESSION_RESULT:", evidence.verdict);
  console.log("Evidence written to:", evidencePath, "sha256:", sha);
  if (evidence.verdict !== "R2R3_ADMIN_SESSION_PASS") {
    process.exit(1);
  }
}
