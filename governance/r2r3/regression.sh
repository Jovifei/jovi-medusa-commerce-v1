#!/usr/bin/env bash
# R2-R3 full regression over the updated audited source tree (76 files).
set -euo pipefail
...
OUT="${R2R3_EVIDENCE_OUT:-$REPO_ROOT/governance/r2r3}"
NS="jovi-medusa-r2r3-reg"
mkdir -p "$OUT"
LOG="$OUT/regression.log"
: > "$LOG"
log(){ echo "[r2r3] $*" | tee -a "$LOG"; }

SRC_SHA="664d73663ffce757bdf394a293c5642720fad5cb0afa1564619f53e845090602"
LOCK_SHA="9855eabfc4fc37d916af0ac64585f15594b44a90dc6d8488d594789956237119"
IMG="jovi-medusa-r2r3-backend:local"
DB="$NS-db"; REDIS="$NS-redis"; BE="$NS-backend"
PGV="pgdata-r2r3"; RDV="redisdata-r2r3"
NET="$NS-internal"
JWT="r2r3-synthetic-jwt-not-production"
COOKIE="r2r3-synthetic-cookie-not-production"
...
docker run -d --name "$DB" --network "$NET" -v "$PGV":/var/lib/postgresql/data \
  -e POSTGRES_USER=postgres -e POSTGRES_DB=jovi_medusa_r2r3 -e POSTGRES_HOST_AUTH_METHOD=trust \
...
for i in $(seq 1 60); do
  if docker run --rm --network "$NET" "$IMG" sh -lc \
    "cd /workspace/apps/backend && node -e \"const {Client}=require('/workspace/node_modules/.pnpm/pg@8.23.0/node_modules/pg');const c=new Client({connectionString:'postgres://postgres@$DB:5432/jovi_medusa_r2r3?sslmode=disable',ssl:false});c.connect().then(()=>c.end()).catch(()=>process.exit(1))\"" \
...
docker run -d --name "$BE" --network "$NET" \
  ...
  -e "DATABASE_URL=postgres://postgres@$DB:5432/jovi_medusa_r2r3?sslmode=disable" \
...
# Jest unit (natural shutdown, 7 suites / 17 tests)
